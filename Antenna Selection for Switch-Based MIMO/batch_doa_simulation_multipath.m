...
function [errors, results, estR] = batch_doa_simulation_multipath(target_all, theta_max, dv, snr_single, L, gain_coe)
    ...
    for i_angle = 1:length(target_all)
        ...
        gain_vec = rand(1,L)*gain_coe;
        yl = zeros(size(y0));
        for multipath_i = 1:L
            theta  = (rand(1,1)-0.5)*2*theta_max/180*pi;       
            a      = exp(1j*pi*dv*sin(theta))';
            a      = sqrt(M)*a/norm(a); 
%             s      = ones(Ns, 1);
            s = gain_vec(multipath_i);
            yl     = yl + a*transpose(s)/sqrt(M);  
        end
        
        y      = awgn(y0 + yl, snr_single, 'measured');
        estR   = zeros(M, M);
        for k=1:Ns
            estR = estR + y(:, k)*y(:, k)';
        end
