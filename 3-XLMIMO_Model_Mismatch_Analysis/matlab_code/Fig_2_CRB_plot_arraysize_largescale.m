...
for xi = 1:length(iter_vec)
    disp([num2str(xi) '/' num2str(length(iter_vec))])
        ...
        cf = c0;
        cf.PU = pos;
        cf.D_Rx = (0:(iter_vec(xi)-1))';  % uniform linear array
        cf.wave_type = "PWM";
        cf = update_parameters(cf);
        cf = get_Rx_symbol(cf);
        cf = get_CRLB(cf);
        ...
        cp = c0;
        cp.PU = pos;
        cp.D_Rx = (0:(iter_vec(xi)-1))';  % uniform linear array
        cp.wave_type = "SWM";
        cp.NF_SNS = "True";
        cp.NF_SWM = "True";
        cp.NF_BSE = "True";
...
figure;plot(iter_vec, E_PEB(:,1), 'k-')
hold on;plot(iter_vec, E_PEB(:,2), 'bo--')
hold on;plot(iter_vec, E_PEB(:,3), 'rs--')
hold on;plot(iter_vec, E_PEB(:,4), 'gd--')
xlabel('Array Size');
ylabel('Relative Mismatch Error [dB]');
legend('LB (TM)', 'LB (only SNS)', 'LB (only SWM)', 'LB (only BSE)', 'Location', 'Southeast')
axis([0 150 -80 0])
grid on;
set(gca,'FontSize', 18);
